...
def get_upcoming_bookings():
    """
    Fetches upcoming bookings for the currently authenticated user.
    Supports pagination, status filtering, and resource name filtering.
    """
    logger = current_app.logger
    try:
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 5, type=int)
        my_bookings_per_page_options = [5, 10, 25, 50] # Consistent with my_bookings
        ...
        check_in_minutes_after = booking_settings.check_in_minutes_after if booking_settings else 15
        if not booking_settings:
             logger.warning("BookingSettings not found, using default check-in window for get_upcoming_bookings.")

        now_utc = datetime.now(timezone.utc)

        # Base query for upcoming bookings
        user_bookings_query = Booking.query.filter(
            Booking.user_name == current_user.username,
            Booking.start_time >= now_utc # Filter for upcoming
        )
        ...
        # Apply sorting for upcoming bookings (ascending by start_time)
        user_bookings_query = user_bookings_query.order_by(Booking.start_time.asc())

        # Paginate the query
        paginated_query = user_bookings_query.paginate(page=page, per_page=per_page, error_out=False)
        upcoming_bookings_from_db = paginated_query.items
        total_items = paginated_query.total
        total_pages = paginated_query.pages

        processed_upcoming_bookings = []
        for booking in upcoming_bookings_from_db:
            resource = Resource.query.get(booking.resource_id) # Consider optimizing if performance is an issue
            resource_name = resource.name if resource else "Unknown Resource"
            ...
            if booking.check_in_token and booking.checked_in_at is None and booking.status == 'approved':
                ...
                # Token is valid if it hasn't expired and the booking hasn't ended
                if token_expires_at_aware and token_expires_at_aware > now_utc and booking_end_time_aware > now_utc:
                    display_check_in_token = booking.check_in_token
            ...
            processed_upcoming_bookings.append(booking_dict)

        logger.info(f"User '{current_user.username}' fetched upcoming bookings. Page {page}/{total_pages}, Items {len(processed_upcoming_bookings)}/{total_items}.")

        return jsonify({
            'success': True,
            'bookings': processed_upcoming_bookings,
            'pagination': {
                'current_page': page,
                'per_page': per_page,
                'total_items': total_items,
                'total_pages': total_pages,
                'per_page_options': my_bookings_per_page_options
            },
            'check_in_out_enabled': enable_check_in_out
        }), 200

    except ValueError as ve: # Handle specific errors like invalid page/per_page
        logger.warning(f"Invalid parameter for get_upcoming_bookings for user '{current_user.username}': {ve}")
        return jsonify({'success': False, 'error': f'Invalid parameter: {str(ve)}'}), 400
    except Exception as e:
        logger.exception(f"Error fetching upcoming bookings for user '{current_user.username}':")
        return jsonify({
            'success': False,
            'error': 'Failed to fetch upcoming bookings due to a server error.'
        }), 500
...
def get_past_bookings():
    """
    Fetches past bookings for the currently authenticated user.
    Supports pagination, status filtering, and resource name filtering.
    """
    logger = current_app.logger
    try:
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 5, type=int)
        my_bookings_per_page_options = [5, 10, 25, 50]
        ...
        booking_settings = BookingSettings.query.first()
        enable_check_in_out = booking_settings.enable_check_in_out if booking_settings else False
        # Note: check_in related times might not be strictly necessary for past bookings display logic,
        # but retaining for consistency or if 'can_check_in' logic were ever adapted for past (unlikely).
        check_in_minutes_before = booking_settings.check_in_minutes_before if booking_settings else 15
        check_in_minutes_after = booking_settings.check_in_minutes_after if booking_settings else 15
        if not booking_settings:
             logger.warning("BookingSettings not found, using default check-in window for get_past_bookings (though likely not applicable).")

        now_utc = datetime.now(timezone.utc)

        # Base query for past bookings
        user_bookings_query = Booking.query.filter(
            Booking.user_name == current_user.username,
            Booking.start_time < now_utc # Filter for past
        )
        ...
        # Apply sorting for past bookings (descending by start_time)
        user_bookings_query = user_bookings_query.order_by(Booking.start_time.desc())

        # Paginate the query
        paginated_query = user_bookings_query.paginate(page=page, per_page=per_page, error_out=False)
        past_bookings_from_db = paginated_query.items
        total_items = paginated_query.total
        total_pages = paginated_query.pages

        processed_past_bookings = []
        for booking in past_bookings_from_db:
            resource = Resource.query.get(booking.resource_id)
            resource_name = resource.name if resource else "Unknown Resource"

            # For past bookings, can_check_in is always False.
            # Also, check_in_token should typically be None or not displayed for past bookings.
            # The logic for `display_check_in_token` from `get_upcoming_bookings` would correctly evaluate
            # to None as `booking_end_time_aware > now_utc` would be false.
            ...
            booking_dict = {
                ...
                'checked_in_at': booking.checked_in_at.replace(tzinfo=timezone.utc).isoformat() if booking.checked_in_at else None,
                'checked_out_at': booking.checked_out_at.replace(tzinfo=timezone.utc).isoformat() if booking.checked_out_at else None,
                'can_check_in': False, # Explicitly false for past bookings
                'check_in_token': None # Explicitly None for past bookings
            }
            processed_past_bookings.append(booking_dict)

        logger.info(f"User '{current_user.username}' fetched past bookings. Page {page}/{total_pages}, Items {len(processed_past_bookings)}/{total_items}.")

        return jsonify({
            'success': True,
            'bookings': processed_past_bookings,
            'pagination': {
                'current_page': page,
                'per_page': per_page,
                'total_items': total_items,
            ...
            'check_in_out_enabled': enable_check_in_out # Still include global setting
        }), 200

    except ValueError as ve:
        logger.warning(f"Invalid parameter for get_past_bookings for user '{current_user.username}': {ve}")
        return jsonify({'success': False, 'error': f'Invalid parameter: {str(ve)}'}), 400
    except Exception as e:
        logger.exception(f"Error fetching past bookings for user '{current_user.username}':")
        return jsonify({
            'success': False,
            'error': 'Failed to fetch past bookings due to a server error.'
        }), 500
