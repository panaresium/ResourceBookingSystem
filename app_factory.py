...
try:
    from azure_backup import perform_startup_restore_sequence, azure_backup_if_changed # Changed here
    azure_backup_available = True
except ImportError:
    perform_startup_restore_sequence = None # Changed here
...
def create_app(config_object=config, testing=False): # Added testing parameter
    ...
    if not testing and azure_backup_available and callable(perform_startup_restore_sequence):
        scheduler_settings = load_scheduler_settings() # load_scheduler_settings is already imported
        ...
        if scheduler_settings.get('auto_restore_full_system_on_startup', False):
            ...
            try:
                ...
                # The detailed import logic for map, resource, user configs is now INSIDE perform_startup_restore_sequence
    ...
    elif not testing: # If azure_backup_available or perform_startup_restore_sequence is not callable
        scheduler_settings = load_scheduler_settings()
        if scheduler_settings.get('auto_restore_full_system_on_startup', False):
             app.logger.warning("Automatic restore on startup is ENABLED in settings, but restore utilities (perform_startup_restore_sequence) are not available. Skipping restore.")
        else:
            app.logger.info("Azure backup utilities not available or perform_startup_restore_sequence not imported. Skipping startup restore.")
