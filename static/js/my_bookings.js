document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchUpcomingBookings() {
        ...
        let url = `/api/bookings/my_bookings?page_upcoming=${upcomingCurrentPage}&per_page_upcoming=${upcomingItemsPerPage}`;
        ...
        try {
            ...
            upcomingTotalItems = data.upcoming_pagination.total_items;
            upcomingTotalPages = data.upcoming_pagination.total_pages;
            upcomingCurrentPage = data.upcoming_pagination.current_page; // Sync with server response
    ...
    async function fetchPastBookings() {
        ...
        let url = `/api/bookings/my_bookings?page_past=${pastCurrentPage}&per_page_past=${pastItemsPerPage}`;
        const status = statusFilterSelect ? statusFilterSelect.value : '';
        const resourceName = resourceNameFilterInput ? resourceNameFilterInput.value.trim() : '';
        if (status) url += `&status_filter=${encodeURIComponent(status)}`;
        if (resourceName) url += `&resource_name_filter=${encodeURIComponent(resourceName)}`;
        ...
        try {
            ...
            pastTotalItems = data.past_pagination.total_items;
            pastTotalPages = data.past_pagination.total_pages;
            pastCurrentPage = data.past_pagination.current_page;
            ...
            if (data.past_bookings && data.past_bookings.length > 0) {
                data.past_bookings.forEach(booking => {
                    const bookingCard = createBookingCardElement(booking, data.check_in_out_enabled);
